...
export default function Header() {
    ...
    const isSpace = pathname.startsWith("/RedFolderGames/SpaceExtraction");
    ...
    const pageLinks = isSpace
        ? [
            { label: "Briefing", target: "station-briefing" },
            { label: "Ship", target: "ship-cutaway" },
            { label: "Station", target: "station-map" },
            { label: "Core", target: "ai-core" }
        ]
        : isUntitled
        ? [
    ...
    return Visible ? (
        <div>
            ...
            ${isSpace ? "sxe-siteHeader" : ""}
            ${!isTTS && !isAE && !isUntitled && !isCity && !isSpace ? "m-5" : ""}`}
            >
                {isHome && (
                    <>
                        <button
                ...
                {(isAE || isUntitled || isCity || isSpace) && (
                    <>
                        <button
                            onClick={() => navigate("/RedFolderGames/")}
                        ...
                        <div className="rfg-pageHeaderStatus">
                            <span>{isAE ? "Carrier Ops" : isCity ? "City Collapse" : isSpace ? "Station Scavenge" : "Raid Island"}</span>
                            <strong>{isAE ? "Fuel Margin Low" : isCity ? "Phase Rising" : isSpace ? "Ship Present" : "No Forced Timer"}</strong>
                        </div>
                    </>
                )}
            </nav>
